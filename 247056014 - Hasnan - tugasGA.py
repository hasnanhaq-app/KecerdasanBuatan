...
def create_individual():
    return [random.randint(0, durasi_maksimal - 1) for _ in range(4)]
...
def evaluate(individual):
    ...
    end_times = {act: start + durasi[act] for act, start in start_times.items()}

    # Syarat 1: A harus sebelum B
    if not (start_times['A'] + durasi['A'] <= start_times['B']):
    ...
    if max(end_times.values()) > durasi_maksimal:
...
def mutate(individual, mutation_rate=0.1):
    for i in range(len(individual)):
        if random.random() < mutation_rate:
            individual[i] = random.randint(0, durasi_maksimal - 1)
